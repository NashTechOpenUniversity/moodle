...
export default class {
    ...
    constructor(editor, modal) {
        ...
        this.allowedDownloadCheckbox = this.modalRoot.querySelector('input[data-action="allowed-download"]');
    ...
    /**
     * Enable the allowed download option.
     *
     * @returns {boolean}
     */
    enableAllowedDownloadOption() {
        return true;
    }
     ...
     * @param {boolean} visible Set the visibility of the button.
     */
    setAllowedDownloadButtonVisibility(visible) {
        const container = this.getButtonContainer('allowed-download');
        container.classList.toggle('hide', !visible);
    }
    ...
    async onMediaStopped() {
        ...
        if (this.enableAllowedDownloadOption()) {
            this.setAllowedDownloadButtonVisibility(true);
        }
    ...
    /**
     * Prepare help strings.
     *
     * @returns {Object}
     */
    static async getHelpStrings() {
        const [allowedDownload] = await getStrings([
            'alloweddownload_help',
        ].map((key) => ({
            key,
            component,
        })));
     ...
     * @returns {Object}
     */
    static async getTemplateContext(editor) {
        const data = {};
    ...
    static async display(editor) {
        ...
        const modal = await ModalClass.create({
            templateContext: await this.getTemplateContext(editor),
            large: true,
            removeOnClose: true,
        });
    ...
    async startRecording() {
        if (this.mediaRecorder) {
            ...
            if (this.hasData()) {
                ...
                this.setAllowedDownloadButtonVisibility(false);
    ...
    getEmbedTemplateContext(templateContext) {
        const mediaAttributes = this.getMediaAttributes();
        return Object.assign({}, templateContext, mediaAttributes);
    }

    /**
     * Retrieves extra media attributes for embed template.
     *
     * @returns {Object}
     */
    getMediaAttributes() {
        if (this.enableAllowedDownloadOption()) {
            if (this.allowedDownloadCheckbox && !this.allowedDownloadCheckbox.checked) {
                return {
                    extraattributes: 'controlslist="nodownload"oncontextmenu="return false;"'
                };
            }
        }
        return {};
