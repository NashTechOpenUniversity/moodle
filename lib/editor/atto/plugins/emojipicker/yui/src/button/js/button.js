...
Y.namespace('M.atto_emojipicker').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _insertEmoji: function(emoji, name) {
        var host = this.get('host'),
            emojiHtml = '<span role="img"' +
                'aria-label="' + name + '"' +
                'data-type="emoji-element">' + emoji + '</span>';
        ...
        host.insertContentAtFocusPoint(emojiHtml);

        // Set focus out of span not emoji.
        var selection = window.rangy.getSelection();
        if (selection.rangeCount > 0) {
            var range = selection.getRangeAt(0);
            var parentElement = range.commonAncestorContainer;
            range.collapseAfter(parentElement);
