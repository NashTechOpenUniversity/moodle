...
Y.namespace('M.atto_image').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    initializer: function() {
        ...
        this.editor.on('dragenter', function(e) {
            this.dragEnterCount++;
            var customEvent = new CustomEvent('dragEnterEditor', {
                bubbles: false,
                cancellable: false
            });
        ...
        this.editor.on('dragleave', function() {
            this.dragEnterCount--;
            if (this.dragEnterCount === 0) {
                var customEvent = new CustomEvent('dragLeaveEditor', {
                    bubbles: false,
                    cancellable: false
                });
                document.dispatchEvent(customEvent);
    ...
    _handleDragDrop: function(e) {
        ...
        var customEvent = new CustomEvent('dropInEditor', {
            bubbles: true,
            cancellable: false
        });
        document.dispatchEvent(customEvent);
