...
var CSS = {
    ...
    REGEX = {
        ISPERCENT: /\d+%/,
        IMAGE_WITHOUT_ALT: /(<img(?![^>]*?\salt=(['"])[^\2]*\2)[^>]*)(>)/g,
        HAS_IMAGE_TAG: /<img[^>]*>/g
    },
...
Y.namespace('M.atto_image').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    initializer: function() {
        ...
        host.on('change', function() {
            var html = host.getCleanHTML();
            if (REGEX.IMAGE_WITHOUT_ALT.exec(html) && REGEX.HAS_IMAGE_TAG.exec(html)) {
                host.showMessage(M.util.get_string('missingalttag', COMPONENTNAME), NOTIFY_WARNING, RECOVER_MESSAGE_TIMEOUT);
            }
        }, this);
    },
