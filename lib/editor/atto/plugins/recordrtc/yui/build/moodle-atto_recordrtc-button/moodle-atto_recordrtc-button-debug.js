    ...
    '<div class="{{PLUGINNAME}} container-fluid">' +
      ...
      '<div class="{{bs_row}}">' +
        ...
        '<div class="{{bs_col}}10">' +
          ...
          '<button id="pause" class="{{bs_pause_btn}}">{{pause}}</button>' +
        '</div>' +
        '<div class="{{bs_col}}1"></div>' +
      '</div>' +
      '<div class="{{bs_row}} hide">' +
...
Y.namespace('M.atto_recordrtc').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _createContent: function(type) {
        var allowedpausing = this.get('allowedpausing');
        ...
        if (allowedpausing) {
            bsPaBtn = 'd-none btn btn-lg btn-outline-primary col-2 ml-3';
        }
        ...
        var bodyContent = Y.Handlebars.compile(TEMPLATE)({
            ...
            pause: M.util.get_string('pause', 'atto_recordrtc'),
    ...
    ATTRS: {
        ...
        /**
         * The allowedpausing to use when generating this recordrtc.
         *
         * @attribute allowedpausing
         * @type Boolean
         */
        allowedpausing: {
            value: null
