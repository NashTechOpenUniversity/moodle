...
M.atto_recordrtc.commonmodule = {
    ...
    handle_stop: function() {
        ...
        // Show allow download checkbox.
        cm.downloadCheckbox.ancestor().ancestor().removeClass('hide');
        ...
        cm.uploadBtn.on('click', function() {
            ...
            if (cm.chunks.length === 0) {
                ...
                if (!cm.downloadCheckbox.get('checked')) {
                    cm.player.set('controlsList', 'nodownload');
                }
    ...
    create_annotation: function(type, recording_url, allowdownload) {
        var html = '';
        ...
        if (type == 'audio') {
            html = "<audio controls='true'" + attr + ">";
        } else { // Must be video.
            html = "<video controls='true'" + attr + ">";
        }
    ...
    insert_annotation: function(type, recording_url) {
        var annotation = cm.create_annotation(type, recording_url, cm.downloadCheckbox.get('checked'));
...
M.atto_recordrtc.audiomodule = {
    init: function(scope) {
        ...
        cm.downloadCheckbox = Y.one('input#allow-download');
        cm.recType = 'audio';
        ...
        cm.startStopBtn.on('click', function() {
            ...
            if ((cm.startStopBtn.get('textContent') === M.util.get_string('startrecording', 'atto_recordrtc')) ||
                ...
                cm.downloadCheckbox.ancestor().ancestor().addClass('hide');
