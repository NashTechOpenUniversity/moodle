...
M.atto_recordrtc.commonmodule = {
    ...
    pause_recording: function() {
        var pauseText = M.util.get_string('pause', 'atto_recordrtc');
        var resumeText = M.util.get_string('resume', 'atto_recordrtc');
        cm.pauseBtn.set('disabled', true);
        if (cm.isPaused) {
            cm.isPaused = false;
            cm.pause_time();
            cm.mediaRecorder.pause();
            cm.pauseBtn.setHTML(resumeText);
            setTimeout(function() {
                cm.pauseBtn.set('disabled', false);
            }, 500);

        } else {
            cm.isPaused = true;
            cm.pauseBtn.setHTML(pauseText);
            cm.resume_time();
            cm.mediaRecorder.resume();
            setTimeout(function() {
                cm.pauseBtn.set('disabled', false);
            }, 500);
        }
    ...
    stop_recording: function(stream) {
        ...
        cm.pauseBtn.replaceClass('d-inline', 'd-none');
    },
...
M.atto_recordrtc.audiomodule = {
    init: function(scope) {
        ...
        cm.pauseBtn = Y.one('button#pause');
        ...
        cm.startStopBtn.on('click', function() {
            ...
            if ((cm.startStopBtn.get('textContent') === M.util.get_string('startrecording', 'atto_recordrtc')) ||
                ...
                if (cm.editorScope.get('allowedpausing')) {
                    cm.startStopBtn.addClass('d-inline col-9');
                    cm.pauseBtn.replaceClass('d-none', 'd-inline');
                    cm.pauseBtn.setHTML(M.util.get_string('pause', 'atto_recordrtc'));
                }
                // Capture audio stream from microphone.
                M.atto_recordrtc.audiomodule.capture_audio(commonConfig);
            } else { // If button is displaying "Stop Recording".
                // First of all clears the countdownTicker.
                cm.startStopBtn.removeClass('col-9');
