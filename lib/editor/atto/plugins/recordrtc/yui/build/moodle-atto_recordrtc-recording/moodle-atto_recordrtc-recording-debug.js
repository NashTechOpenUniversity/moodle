...
M.atto_recordrtc.commonmodule = {
    ...
    start_recording: function(type, stream) {
        ...
        if (type === 'audio' && cm.editorScope.get('audiortcmp3')) {
            cm.mediaRecorder = new window.mp3MediaRecorder(stream, {worker: new Worker(window.workerURL)});
        } else {
    ...
    upload_to_server: function(type, callback) {
        ...
        xhr.onload = function() {
            if (xhr.status === 200) { // If src media was successfully retrieved.
                ...
                var audioformat = cm.editorScope.get('audiortcmp3') ? '-audio.mp3' : '-audio.ogg';
                fileName += (type === 'audio') ? audioformat
                                               : '-video.webm';
