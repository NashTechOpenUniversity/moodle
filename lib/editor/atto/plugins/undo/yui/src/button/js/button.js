...
Y.namespace('M.atto_undo').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
     ...
     * @method _addToRedo
     * @private
     * @param {Array} stackresult The stack of current node and selection.
     */
    _addToRedo: function(stackresult) {
        this._redoStack.push(stackresult);
    },
     ...
     */
    _addToUndo: function(stackresult, clearRedo) {
        var last = this._undoStack[this._undoStack.length - 1];

        if (typeof clearRedo === 'undefined') {
            clearRedo = false;
        }
        if ((last && stackresult.currentnode.innerHTML !== last.currentnode.innerHTML) || typeof(last) === "undefined") {
            var currentNode = this.editor.getDOMNode();
            var savedSel = window.rangy.saveSelection();
            var clone = currentNode.cloneNode(true); // true tells it to do a deep clone.
            stackresult  = {currentnode: clone, currentsel: savedSel};
            this._undoStack.push(stackresult);
     ...
     * @method _getRedo
     * @private
     * @return {Array} The stack of current node and selection. to restore, or undefined.
     */
    _getRedo: function() {
     ...
     * @method _getUndo
     * @private
     * @param {Array} current The current stack of current node and selection.
     * @return {Array} The HTML to restore.
     */
    _getUndo: function(current) {
        ...
        if (last.currentnode.innerHTML === current.currentnode.innerHTML) {
     ...
     * @param {Array} stackresult The stack of current node and selection.
     */
    _restoreValue: function(stackresult) {
        this.editor.getDOMNode().innerHTML = stackresult.currentnode.innerHTML;
        window.rangy.restoreSelection(stackresult.currentsel);
        window.rangy.removeMarkers(stackresult.currentsel);
    ...
    _undoHandler: function(e) {
        ...
        var clone = currentNode.cloneNode(true); // true tells it to do a deep clone.
        var savedSel = window.rangy.saveSelection();
        var stackresult  = {currentnode: clone, currentsel: savedSel};
        var undo = this._getUndo(stackresult);
        // Edge case, but that could happen. We do nothing when the content equals the undo step.
        if (stackresult.currentnode.innerHTML === undo.currentnode.innerHTML) {
            this._updateButtonsStates();
            return;
        }
        // Restore the value.
    ...
    _changeListener: function(e) {
        ...
        var savedSel = window.rangy.saveSelection();
        var stackresult  = {currentnode: clone, currentsel: savedSel};
        this._addToUndo(stackresult, true);
        this._updateButtonsStates();
    }
});
