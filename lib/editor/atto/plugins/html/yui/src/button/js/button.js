...
Y.namespace('M.atto_html').Button = Y.Base.create('button', Y.M.editor_atto.EditorPlugin, [], {
    ...
    _showCodeMirror: function() {
        var host = this.get('host');
        var element = document.getElementById('fitem_' + host.get('elementid'));
        var isRequired = element.querySelectorAll("[title='Required']").length;
        ...
        this._codeMirror.on('change', function(cm) {
            cm.save();
            cm.triggerValidationFailureEvent(host.get('elementid'));
        });
