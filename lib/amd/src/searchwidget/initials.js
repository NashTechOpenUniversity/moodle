...
/**
 * A small dropdown to filter users within the report.
 *
 * @module    core/searchwidget/initials
 ...
 * @param {Null|Number} filter_userid The user id to filter by.
 * @param {Null|String} filter_search The search value to filter by.
 * @param {String} reportMode Report mode option.
 * @param {Object} extraParams Additional parameters for url.
 */
export const init = (callingLink, filter_userid = null,
        filter_search = null, reportMode = '', extraParams = null) => {
    if (registered) {
        return;
    }
    const pendingPromise = new Pending();
    registerListenerEvents(callingLink, filter_userid, filter_search, reportMode, extraParams);
 ...
 * @param {Object} extraParams Additional parameters for url.
 */
const registerListenerEvents = (callingLink, filter_userid = null,
        filter_search = null, reportMode, extraParams) => {
    const events = [
        'click',
        ...
        document.addEventListener(event, (e) => {
            ...
            if (e.target.closest(`${selectors.formDropdown}`) && e.target.type === selectors.formItems.type) {
                if (e.target.dataset.action === selectors.formItems.save) {
                    ...
                    params = {
                        ...params,
                        'gpr_search': filter_search !== null ? filter_search : '',
                        [prefix + 'first']: sifirst.parentElement.classList.contains('initialbarall') ? '' : sifirst.value,
                        [prefix + 'last']: silast.parentElement.classList.contains('initialbarall') ? '' : silast.value,
                    };
                    if (filter_userid !== null && filter_userid !== '') {
                        params.gpr_userid = filter_userid;
                    }
