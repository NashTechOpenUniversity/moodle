 ...
 * @param {Null|Number} filterUserid The user id to filter by.
 * @param {Null|String} filterSearch The search value to filter by.
 * @param {Null|Number} cmid Course module id.
 * @param {Object} extraParams Additional parameters for url.
 */
export const init = (callingLink, filterUserid = null,
        filterSearch = null, cmid = null, extraParams = null) => {
    if (registered) {
        return;
    }
    const pendingPromise = new Pending();
    registerListenerEvents(callingLink, filterUserid, filterSearch, cmid, extraParams);
 ...
 * @param {Object} extraParams Additional parameters for url.
 */
const registerListenerEvents = (callingLink, filterUserid = null,
        filterSearch = null, cmid, extraParams) => {
    const events = [
        'click',
        ...
        document.addEventListener(event, (e) => {
            ...
            if (e.target.closest(`${selectors.formDropdown}`) && e.target.type === selectors.formItems.type) {
                if (e.target.dataset.action === selectors.formItems.save) {
                    ...
                    params = {
                        ...params,
                        'gpr_search': filterSearch !== null ? filterSearch : '',
                    ...
                    if (filterUserid !== null && filterUserid !== '' && filterUserid !== 0) {
                        params.gpr_userid = filterUserid;
                    }
