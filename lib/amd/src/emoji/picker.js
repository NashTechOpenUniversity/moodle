    ...
    const context = {
        ...
        emojis: emojis.map(emojiData => {
            ...
            return {
                ...
                name: emojiData.name.toLowerCase(),
...
const getHandleClick = (
    ...
    return (e, rowData, categoryScrollPositions) => {
        ...
        if (isEmojiElement(target)) {
            ...
            const emojiData = {unified, shortnames, name};
            ...
            // Call the client's callback function with the selected emoji and emoji name.
            selectCallback(target.textContent, name);
        ...
        if (searchTerm) {
            ...
            const matchingEmojis = Object.keys(EmojiData.byShortName).reduce((carry, shortName) => {
                if (shortName.includes(searchTerm)) {
                    let emojiName = '';
                    EmojiData.byCategory.forEach(category => {
                        const emojiResult = category.emojis.find(emoji => emoji.shortnames.includes(shortName));
                        if (emojiResult) {
                            emojiName = emojiResult.name;
                        }
                    });
                    carry.push({
                        name: emojiName,
