...
/**
 * Web service to get users and extra fields in quiz report.
 *
 * @package    mod_quiz
...
class get_users_in_report extends external_api {
     ...
     * @return array User list and the extra fields.
     */
    public static function execute(int $cmid, string $mode, string $params): array {
        ...
        // We don't need to search user by id.
        $userfieldsapi = \core_user\fields::for_identity($context)->with_userpic()->excluding('id');
        $allowfieldswithpicture = $userfieldsapi->get_required_fields();
        $useridstmp = [];
        $users = array_reduce($table->rawdata, function($response, $user) use ($PAGE, $allowfieldswithpicture, &$useridstmp) {
            // We need to remove duplicate users in the report.
            if (!in_array($user->userid, $useridstmp)) {
                $userref = new \stdClass();
                foreach ($allowfieldswithpicture as $field) {
