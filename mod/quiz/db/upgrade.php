...
function xmldb_quiz_upgrade($oldversion) {
    ...
    if ($oldversion < 2024071100) {
        ...
        // Define table quiz_grade_item_feedback to be created.
        $table = new xmldb_table('quiz_grade_item_feedback');

        // Adding fields to table quiz_grade_item_feedback.
        $table->add_field('id', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        $table->add_field('quizid', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, null);
        ...
        // Adding keys to table quiz_grade_item_feedback.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);
        $table->add_key('gradeitemid', XMLDB_KEY_FOREIGN, ['gradeitemid'], 'quiz_grade_items', ['id']);
        $table->add_key('quizid', XMLDB_KEY_FOREIGN, ['quizid'], 'quiz', ['id']);

        // Conditionally launch create table for quiz_grade_item_feedback.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // Quiz savepoint reached.
        upgrade_mod_savepoint(true, 2024071100, 'quiz');
    }
